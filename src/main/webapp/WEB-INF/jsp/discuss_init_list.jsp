...
<html>
  <head>
    ...
    <script type="text/javascript" src="<%=path%>/js/dc/discuss_init_list.js"></script>
  ...
  <div class="content">
    ...
    <form id="pageInfo_id" method="post" action="<%=path%>/bms/discuss_discussInitList.action"   enctype="multipart/form-data">
        <input type="hidden" name="discussManagerForm.type" value="${discussManagerForm.type}" />
	      ...
	      <td>操作</td>
		    ...
		    <c:forEach var="list" items="${list}" >
		      <tr>
				      ...
				      <td>
				      <fmt:formatDate value="${list.insert_date}" pattern="yyyy-MM-dd HH-mm"/>
				      </td>
				      <td>${list.sponsor}</td>
				      <td>${list.real_name}</td>
				      <td>${list.status}</td>
				      <td>
				      <input class="btn_sq" type="button" value="初始化"  onclick="initDiscussToAdd('${list.id}');" />
